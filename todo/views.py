...
def loginuser(request):
	if request.method == 'GET':
		...
		if user is None:
			return render(request, 'todo/login.html', {'form': AuthenticationForm(), 'error': 'Username and password did not matched'} )
		else:
			login(request, user)
			return redirect('currenttodos')
...
def viewtodo(request, todo_pk):
	...
	if request.method == 'GET':
		...
		try:
			...
			return render(request, 'todo/viewtodo.html',{'todos':todo, 'form': form, 'error': 'There is a value error'})
@login_required
